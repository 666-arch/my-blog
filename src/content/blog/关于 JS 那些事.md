...
> OSS是阿里云提供的一套文件云对象存储库，传统的文件上传是直接将文件以 目录 -> 文件 形式传递给服务端，根据服务端创建的目录进行存储

Q：场景应用

> 在多文件存储功能中，可以将大量文件存储到OSS云对象存储库中，它不再是以 目录 -> 文件形式的存储

它是分成多个 `bucket` 的结构形式存放不同的文件，开发者可以根据情况存储

比如 `bucket-1`，`bucket-2`，可以理解为每一个 bucket 都是一个 ”桶“

#### OSS 大致分为三种存储方式

- 块存储：相当于把磁盘给你，你自己格式化，容量有限
- 文件存储：以目录文件层次结构存储，你可以自己上传或者下周，容量有限
- 对象存储：Key-value 形式，分布式实现的，容量无限（绝对多数情况使用这种方式）
